...
export default function Detail() {
  ...
  const fetchTotalRating = async () => {
    try {
      const res = await STAFF.getTotalRating(id.id);
      if (res.status === 200) {
  ...
  return (
    <div className="flex flex-col max-h-[75vh] overflow-y-auto">
      ...
      {
        totalRating.length > 0 && <div className="flex flex-col border border-gray-200 rounded-md">
          <div className="flex justify-center text-center text-xl">
            Tổng quan đánh giá
          </div>
          <div className="flex flex-col justify-center items-center">
            <Rating name="read-only" value={4.7} precision={0.1} readOnly/>
            <div>{data?.avg_rating}/5 (123 đánh giá)</div>
          </div>
