...
const List = () => {
  ...
  const handleActive = async (id) => {
    const res = await STAFF.changeAciveService(id);
    if (res.status === 200) {
      toast.success("Cập nhật trạng thái thành công");
      data.map((item) => {
        if (item.id === id) {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, {type}) => <>{type === serviceType.DOCTOR ? <Tag color="green">Bác sĩ</Tag> :
        <Tag color="red">Gói khám</Tag>}</>, // <Tag color={status ? "green" : "red"}>{status ? "Hiện" : "Chủ biểt"}</>,
    },
    {
      title: 'Trang thái',
      dataIndex: 'active',
      ...
      render: (_, {active, id}) => {
        return (
          <>
            <Button
              sx={{marginRight: 1}}
              variant="outlined" color={active ? "error" : "success"}
  ...
  return (
    <div className="max-h-[75vh] overflow-y-auto relative">
      <div className="w-full flex h-10 my-auto justify-start gap-3 sticky top-0 z-10 bg-white mb-2">
        <div className="w-28 text-center my-auto font-bold">Tìm kiếm</div>
        <input
          type="text"
          value={search}
          onChange={(e) => {
